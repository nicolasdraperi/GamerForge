...
class Character(models.Model):
    game_concept = models.ForeignKey(GameConcept, on_delete=models.CASCADE, related_name="characters_set")
    name = models.CharField(max_length=100)
    role = models.CharField(max_length=100)
    description = models.TextField(blank=True, null=True)

    def __str__(self):
        return f"{self.name} ({self.game_concept.title})"


class Location(models.Model):
    game_concept = models.ForeignKey(GameConcept, on_delete=models.CASCADE, related_name="locations_set")
    name = models.CharField(max_length=100)
    description = models.TextField(blank=True, null=True)
